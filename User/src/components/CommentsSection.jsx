...
const CommentsSection = ({ videoId }) => {
    const { isDarkMode, apiBaseUrl, username, userid } = useShared();
    ...
    const handleCommentLike = (id) => {
        if (id) {
            ...
            .then(function (response) {
                getComments(); // Refresh comments to update likes
            })
            .catch(function (error) {
                console.log(error);
            });
    ...
    const isLikedByUser = (likes, userId) => likes.includes(userId);

    return (
        <div className={`mt-4 ${isDarkMode ? 'bg-gray-800' : 'bg-white'} p-4 rounded-lg flex-1`}>
            <h2 className={`text-2xl font-bold ${isDarkMode ? 'text-gray-200' : 'text-gray-800'} mb-4`}>
                    ...
                    <div key={index} className={`${isDarkMode ? 'bg-gray-900' : 'bg-gray-100'} p-2 rounded-md`}>
                        ...
                        <div className="flex flex-row mt-2">
                            <div className='flex-1 flex flex-row'>
                                ...
                                >
                                    {isLikedByUser(commentData.likedBy, userid) ? 
                                        <ThumbUpSolid className="h-5 w-5" /> : 
                                        <ThumbUpIcon className="h-5 w-5" />}
                                    <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                                        {commentData.initialLike}
                                    </p>
                                </button>
                                <button
                        ...
                        <div className="flex justify-between mt-2">
                          <div className='flex flex-row'>
                            ...
                            >
                              ...
                              {isLikedByUser(reply.likedBy, userid) ? 
                                        <ThumbUpSolid className="h-5 w-5" /> : 
                                        <ThumbUpIcon className="h-5 w-5" />}
                              {console.log("r : ",reply)}
